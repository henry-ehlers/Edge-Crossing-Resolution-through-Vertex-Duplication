...
def check_sight_cells(face_incidence_table, target_vertices,
    ...
    for selected_face in selected_faces:
        ...
        if not is_outer:
            ...
            # If the face is convex, define its sight cell simply in terms of the face
        ...
        # Get Sight Cells of Outer Face
            ...
            cell_incidence_table, new_graph = get_outer_face_sight_cells(outer_faces=outer_faces,
                                                                         ...
                                                                         positions=positions,
                                                                         index=index)
        print(f"cell incidences:")
        print(cell_incidence_table)

        # TODO: update the graph with the new graphs obtained from face selection
        # TODO: update selected faces with sight cells for selection next time around
        #  IF THE FACE COULD NOT BE REALIZED -> RETURN AND RESELECT

        # If NO SINGLE SIGHT CELL WAS RETURNED -> UPDATE THE GRAPH AND FACE TABLE
        if len(cell_incidence_table.index) > 1:
            update_graph_with_sight_cells(graph=graph,
                                          positions=positions,
                                          edges_inner=sorted_inner_face_edges,
                                          edges_outer=sorted_outer_face_edges,
                                          new_graph=new_graph[3],
                                          new_positions=new_graph[4],
                                          new_edge_map=new_graph[2])

        if face_incidence - cell_incidence_table.loc[0].at["incidence"]:
            # premature return -> return to sorting and selection step
            pass

    return None


# TODO: create graph with outer face decomposed -> all faces are now inner faces
def get_decomposed_outer_face():
    pass
...
# TODO: build the inner face decomposition
def get_inner_face_sight_cells(face, sorted_face_edges, is_cycle, target_vertices, graph, positions):
    pass
...
def update_graph_with_sight_cells(graph, positions, edges_inner, edges_outer, new_graph, new_positions, new_edge_map,
                                  outer=False):
    print(f'\nOld Edges: {graph.edges()}')
    print(f"Old Nodes: {graph.nodes()}")

    print(f'\nNew Edges: {new_graph.edges()}')
    print(f"New Nodes: {new_graph.nodes()}")
